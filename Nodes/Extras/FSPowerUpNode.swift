...
class FSPowerUpNode: SKShapeNode {
    ...
    init(type: FSPowerUpType, size: CGSize) {
        self.type = type
        self.size = size
        super.init()
        setup()
...
// MARK: Helpers
extension FSPowerUpNode {
    
    func pauseAnimation() { isPaused = true }
    func resumeAnimation() { isPaused = false }
    ...
    private func setup() {
        ...
        physicsBody?.categoryBitMask = FSPhysicsCategory.PowerUp
        physicsBody?.contactTestBitMask = FSPhysicsCategory.Ball
