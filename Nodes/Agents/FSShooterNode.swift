...
class FSShooterNode: SKNode {
    let shooterBody: SKSpriteNode
    private let aimLine: FSTargetLineNode
    ...
    init(ballRadius: CGFloat) {
        ...
        aimLine = FSTargetLineNode(ballRadius: ballRadius)
...
extension FSShooterNode {
    ...
    private func setupThunder() {
        ...
        thunder?.position = CGPoint(x: shooterBody.size.width * 0.4, y: -shooterBody.size.height * 0.4)
        thunder?.zPosition = 1
...
extension FSShooterNode {
    ...
    private func updateLaserSight() {
        guard let scene = scene as? FSGameScene else { return }
        ...
        for _ in 0..<maxBounces {
            ...
            totalDistance += currentPoint.fruitShootDistance(to: nextPoint)
    ...
    private func drawWavyLineSegment(from start: CGPoint, to end: CGPoint, startDistance: CGFloat, path: CGMutablePath) {
        let segmentLength = start.fruitShootDistance(to: end)
        let direction = CGPoint(x: end.x - start.x, y: end.y - start.y).fruitShootNormalized()
    ...
    private func calculateNextPoint(from point: CGPoint, angle: CGFloat, in scene: FSGameScene) -> (point: CGPoint, angle: CGFloat, hitBlock: Bool) {
        let dx = cos(angle)
        let dy = sin(angle)
        ...
        for corner in corners {
            if let cornerCollision = rayIntersectsCircle(center: corner, radius: radius, rayOrigin: center, rayDirection: direction) {
                if cornerCollision.t < rectCollision.t {
                    let normal = CGPoint(x: cornerCollision.point.x - corner.x, y: cornerCollision.point.y - corner.y).fruitShootNormalized()
...
extension CGPoint {
    func fruitShootDistance(to point: CGPoint) -> CGFloat {
        return sqrt(pow(x - point.x, 2) + pow(y - point.y, 2))
    }
    
    func fruitShootNormalized() -> CGPoint {
