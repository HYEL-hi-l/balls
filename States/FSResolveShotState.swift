...
class FSResolveShotState: FSGameState {
    
    private var firstBottomBallPosition: CGPoint?
    
    override func isValidNextState(_ stateClass: AnyClass) -> Bool {
        return stateClass is FSIdleState.Type || stateClass is FSGameOverState.Type
    }
            ...
            self.shiftBlocksDown() {
                if self.isGameOver() {
                    ...
                    dispatchGroup.notify(queue: .main){
                        self.stateMachine?.enter(FSGameOverState.self)
                    }
                } else {
                    self.addNewBlockRow() {
                        self.updateGameInfo()
                        ...
                        self.stateMachine?.enter(FSIdleState.self)
                    }
                }
            }
        }
        ...
        let dispatchGroup = DispatchGroup()
            for i in 0..<numberOfBlocks {
                ...
                if i == bonusBallIndex {
                    ...
                    let blockNode = FSBlockNode(size: blockSize, hitPoints: hitPoints, atlas: gameScene.blocksAtlas)
